...
def set_up_personal_training_page(request):
    if request.method == 'GET':
        ...
        future_booked_sessions = get_future_personal_training_sessions(
            request.user)

        context = {
            'times_list': TIMES_LIST,
            'booked_sessions': future_booked_sessions,
            'session_message': ""
        }
...
def create_personal_training_session(request):
    personal_training_booked_sessions = get_future_personal_training_sessions(
        request.user)
    ...
    if request.method == 'GET':  # If the form has been submitted...
        ...
        isDateLater = is_date_later_than_or_equal_to_todays_date(
            filter_datetime)
...
def update_personal_training_session(request):
    ...
    if request.GET:
        ...
        try:
            ...
            context["personal_training_data"] = {booked_session}
            context["date_message"] = convert_datetime_for_display(
                new_datetime)
